...
const html=(body,status=200,extraHeaders={})=>new Response(body,{status,headers:{"content-type":"text/html; charset=utf-8","cache-control":status>=500?"no-store":"public, max-age=60, stale-while-revalidate=300","x-content-type-options":"nosniff","referrer-policy":"strict-origin-when-cross-origin",...extraHeaders}});
...
const publicFilter='status == "published" && defined(category->_id) && count(body) > 0 && length(slug.current) > 0 && length(title) > 0 && length(summary) > 0 && defined(dateTime(publishedAt)) && dateTime(publishedAt) <= dateTime(now())';
...
const isPublic=(article,now=new Date())=>{const publishedAt=article&&new Date(article.publishedAt);return Boolean(article&&article.status==="published"&&typeof article.slug==="string"&&article.slug.trim()&&typeof article.title==="string"&&article.title.trim()&&typeof article.summary==="string"&&article.summary.trim()&&typeof article.category==="string"&&article.category.trim()&&Array.isArray(article.body)&&article.body.length>0&&!Number.isNaN(publishedAt.getTime())&&publishedAt<=now)};
...
function reportSanityFailure(area,error){console.error("[editorial] Sanity unavailable in "+area+": "+(error instanceof Error?error.message:"unknown error"))}
...
function renderInline(block){return (block.children||[]).map(child=>{let value=E(child.text||"");(child.marks||[]).forEach(mark=>{if(mark==="strong")value="<strong>"+value+"</strong>";if(mark==="em")value="<em>"+value+"</em>";const def=(block.markDefs||[]).find(d=>d._key===mark&&d._type==="link");if(def)value='<a href="'+E(def.href)+'" rel="noreferrer">'+value+'</a>';});return value;}).join("")}
function renderListBlocks(blocks){const roots=[],stack=[];blocks.forEach(block=>{const level=Math.max(1,Math.min(6,Number(block.level)||1)),node={block,children:[]};while(stack.length&&stack[stack.length-1].level>=level)stack.pop();const target=stack.length?stack[stack.length-1].node.children:roots;target.push(node);stack.push({level,node})});const renderNodes=nodes=>{let result="",index=0;while(index<nodes.length){const tag=nodes[index].block.listItem==="number"?"ol":"ul",group=[];while(index<nodes.length&&(nodes[index].block.listItem==="number"?"ol":"ul")===tag)group.push(nodes[index++]);result+="<"+tag+">"+group.map(node=>"<li>"+renderInline(node.block)+renderNodes(node.children)+"</li>").join("")+"</"+tag+">"}return result};return renderNodes(roots)}
function renderBlocks(blocks=[]){let output="",index=0;while(index<blocks.length){const block=blocks[index];
  if(block&&block._type==="block"&&block.listItem){const list=[];while(index<blocks.length&&blocks[index]&&blocks[index]._type==="block"&&blocks[index].listItem)list.push(blocks[index++]);output+=renderListBlocks(list);continue}
  index+=1;if(!block)continue;
  if(block._type==="divider"){output+="<hr>";continue}
  if(block._type==="image"){output+=cover({coverImage:block},true);continue}
  if(block._type!=="block")continue;
  const tag={h2:"h2",h3:"h3",blockquote:"blockquote"}[block.style]||"p";output+="<"+tag+">"+renderInline(block)+"</"+tag+">"}return output}
const NAV_SCRIPT='document.querySelectorAll(".menu-button").forEach(function(button){var nav=button.closest(".site-header").querySelector("nav");if(!nav)return;button.addEventListener("click",function(){var open=nav.classList.toggle("open");button.setAttribute("aria-expanded",String(open));button.setAttribute("aria-label",open?"Fechar menu":"Abrir menu")});nav.querySelectorAll("a").forEach(function(link){link.addEventListener("click",function(){nav.classList.remove("open");button.setAttribute("aria-expanded","false");button.setAttribute("aria-label","Abrir menu")})})});';
function shell(content,title,description,canonical,ogType="website",ogImage="",schema=""){
  ...
  return '<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">'+meta+'<style>'+CSS+'/*__EDITORIAL_CSS__*/</style></head><body class="articles-page">'+content+'<script>'+NAV_SCRIPT+'<\\/script></body></html>';
}
function header(){return '<a class="skip-link" href="#conteudo">Ir para o conteúdo</a><header class="site-header"><a class="brand" href="/" aria-label="Sady Santana — início"><span class="leaf-mark"><i></i><i></i><i></i><i></i><b></b></span><span><strong>Sady Santana</strong><small>escritora</small></span></a><button class="menu-button" type="button" aria-label="Abrir menu" aria-expanded="false"><span></span><span></span></button><nav aria-label="Navegação principal"><a href="/#sobre">Sobre</a><a href="/#livros">Livros</a><a href="/#palestras">Palestras</a><a href="/artigos" aria-current="page">Artigos</a></nav></header>'}
...
function unavailable(url){const content=header()+'<main id="conteudo" class="not-found"><p class="kicker">Indisponibilidade temporária</p><h1>Não foi possível carregar os artigos agora.</h1><p>Tente novamente em alguns instantes ou volte à página inicial.</p><a href="/">Voltar à home</a></main>'+footer();return html(shell(content,"Artigos temporariamente indisponíveis | Sady Santana","O acervo está temporariamente indisponível.",url.origin+url.pathname).replaceAll("/*__EDITORIAL_CSS__*/",EDITORIAL_CSS),503,{"retry-after":"60"})}
...
async function archivePage(url,env,page=1){
  let articles;try{articles=await sanity(env,'*[_type == "article" && '+publicFilter+'] | order(publishedAt desc) '+projection)}catch(error){reportSanityFailure("archive",error);return unavailable(url)}
  if(!Array.isArray(articles)){reportSanityFailure("archive",new Error("Invalid article collection"));return unavailable(url)}articles=articles.filter(article=>isPublic(article));
...
async function articlePage(slug,env,url){
  let article;try{article=await sanity(env,'*[_type == "article" && slug.current == '+JSON.stringify(slug)+' && '+publicFilter+'][0] '+projection)}catch(error){reportSanityFailure("article",error);return unavailable(url)}
  if(!article||!isPublic(article))return html(shell(header()+'<main id="conteudo" class="not-found"><h1>Este artigo não foi encontrado.</h1><a href="/artigos">Voltar ao acervo</a></main>'+footer(),"Artigo não encontrado | Sady Santana","O artigo solicitado não está disponível.",url.origin+url.pathname).replaceAll("/*__EDITORIAL_CSS__*/",EDITORIAL_CSS),404);
  ...
  const related=isPublic(article.related)?'<aside class="related"><p>Continue lendo</p><a href="/artigos/'+E(article.related.slug)+'">'+E(article.related.title)+'</a></aside>':"";
